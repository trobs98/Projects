...
public class PublisherFactory {
	 ...
	 * 
	 * @param publisherType
	 *            an entry from the {@link PublisherType} enumeration
	 * @param strategyName
	 *            an entry from the {@link StrategyName} enumeration
	...
	public static AbstractPublisher createPublisher(PublisherType publisherType, StrategyName strategyName) {
		...
		case alphaPub:
			ip = new alphaPub(StrategyFactory.createStrategy(strategyName), PublisherIDMaker.getNewPubID()).alphaPub;
			System.out.println("Publisher with ID: " + ip.getID() + " has strategy: " + strategyName);
			return ip;
		case betaPub:
			ip = new betaPub(StrategyFactory.createStrategy(strategyName), PublisherIDMaker.getNewPubID()).betaPub;
			System.out.println("Publisher with ID: " + ip.getID() + " has strategy: " + strategyName);
			return ip;
		case gammaPub:
			ip = new gammaPub(StrategyFactory.createStrategy(strategyName), PublisherIDMaker.getNewPubID()).gammaPub;
			System.out.println("Publisher with ID: " + ip.getID() + " has strategy: " + strategyName);
			return ip;
		case deltaPub:
			ip = new deltaPub(StrategyFactory.createStrategy(strategyName), PublisherIDMaker.getNewPubID()).deltaPub;
			System.out.println("Publisher with ID: " + ip.getID() + " has strategy: " + strategyName);
			return ip;
		default:
			ip = new alphaPub(StrategyFactory.createStrategy(strategyName), PublisherIDMaker.getNewPubID()).alphaPub;
			System.out.println("Default publisher created, alphaPub created");
			System.out.println("Publisher with ID: " + ip.getID() + " has strategy: " + strategyName);
			return ip;
		}
	}
